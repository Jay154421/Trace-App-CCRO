...
export const ATTENDANT_RADIO_CHECK_MARK = {
  widthIn: 12 / 96,
  heightIn: 12 / 96,
  lineWidthIn: 0.024,
};
...
export const ADDRESS_ABBREV_FIELD_KEYS = new Set([
...
export const PRINT_STYLES_ID = 'field-position-print';
 ...
 */
export function abbreviateAddressText(value) {
  if (!value) return '';
 ...
 */
export function parseDateOfBirth(dateStr) {
 ...
 */
export function pickPdfFontPtForColumn(doc, line, maxWidthIn, preferredPt) {
...
export function drawRadioMark(doc, x, y) {
 ...
 */
export function buildCombinedPdfBase64(merged) {
  const doc = new jsPDF({ orientation: 'portrait', unit: 'in', format: 'legal' });
  doc.setFont('helvetica');
  doc.setFontSize(PDF_LAYOUT.pdfDefaultFontSize);
  doc.setTextColor(0, 0, 0);
 ...
 */
export function generateTableWithCheckmarks(doc, data, columns, options = {}, checkmarkColumns = {}) {
