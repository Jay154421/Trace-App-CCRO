...
import {
  OUT_OF_TOWN_PDF_PAGE_FORMAT,
  buildOutOfTownPdfBase64,
  buildOutOfTownSuggestedFilename,
} from '../../utils/outOfTownPdf';
import {
  applyAusfBodyFit,
...
import OutOfTownPrintDocument from './outOfTownPrintDocument';
import { OutOfTownAffidavit } from './OutOfTownAffidavit';

function fitOutOfTownDocToArticle(article, doc) {
  clearAusfFitForPdfCapture(doc);
  applyAusfBodyFit(article, doc);
}

function useOutOfTownFitToPaper(docArticleRef, paperSize, printData, enabled = true) {
  useLayoutEffect(() => {
    ...
    const fit = () => {
      ...
      fitOutOfTownDocToArticle(article, doc);
    };
...
function waitForOutOfTownLayout() {
  return new Promise((resolve) => {
    requestAnimationFrame(() => requestAnimationFrame(resolve));
  });
}

async function prepareOutOfTownPdfCapture(articleRef) {
  const article = articleRef.current;
  const doc = article?.querySelector('.ausf-doc.print-doc');
  if (!article || !doc) return;
  clearAusfFitForPdfCapture(doc);
  await waitForOutOfTownLayout();
  applyAusfBodyFit(article, doc);
  await waitForOutOfTownLayout();
}

export function OutOfTownPrint() {
  const { id } = useParams();
  ...
  const variantLabel = OUT_OF_TOWN_VARIANT_LABEL[variant] || 'Out-of-Town Affidavit';

  const printData = useMemo(() => {
    const stored = child?.out_of_town_affidavit;
    return stored && typeof stored === 'object' ? stored : {};
  }, [child]);

  useOutOfTownFitToPaper(docArticleRef, paperSize, printData, !isEditing);
  ...
  const runPdfExport = useCallback(async () => {
    ...
    try {
      await waitForOutOfTownLayout();
      await prepareOutOfTownPdfCapture(docArticleRef);
      return await buildOutOfTownPdfBase64(article, paperSize);
    } finally {
      document.body.classList.remove('pdf-capture');
      const doc = article?.querySelector('.ausf-doc.print-doc');
      if (article && doc) fitOutOfTownDocToArticle(article, doc);
    }
  }, [paperSize]);
  ...
  const handleSavePdf = useCallback(async () => {
    ...
    try {
      ...
      const suggestedFilename = buildOutOfTownSuggestedFilename(child, paperSize);
      const result = await window.electron.saveFieldPositionPdf(base64, suggestedFilename);
      if (notifyElectronSavePdfResult(result)) {
        onPdfSavedOpenInBrowser(result.filePath, 'Out-of-town affidavit PDF');
      }
    } catch (err) {
  ...
  if (!shouldShowOutOfTownAffidavit(child)) {
    return (
      <p className="m-6 rounded-lg border border-amber-200 bg-amber-50 p-4 text-amber-900">
        This applicant is not marked as out of town. Update the applicant profile to enable this affidavit.
      </p>
    );
  }
    ...
    <section className="witness-affidavit-print ausf-print-page min-h-screen bg-slate-100 print:bg-white">
      {pdfPreviewOpen && pdfPreviewUrl ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="out-of-town-pdf-preview-title"
        >
          <button
            type="button"
          ...
          <div className="relative z-10 flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
            <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-slate-200 px-4 py-3">
              <h2 id="out-of-town-pdf-preview-title" className="text-sm font-semibold text-slate-800">
                PDF preview — {variantLabel}
              </h2>
              <button
                type="button"
                onClick={closePreview}
            ...
            <iframe
              src={pdfPreviewUrl}
              title="Out-of-town affidavit PDF preview"
      ...
      <header className="print:hidden mx-auto max-w-[210mm] px-4 py-4">
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div>
            <h1 className="text-lg font-semibold text-slate-800">{variantLabel}</h1>
            <p className="mt-1 text-sm text-slate-600">Affidavit with corroboration for out-of-town applicant (Legal Office).</p>
            ...
            <fieldset className="m-0 min-w-0 border-0 p-0">
              ...
              <div className="flex flex-wrap items-center gap-3 text-sm text-slate-800">
                ...
                <label className="flex cursor-pointer items-center gap-1.5">
                  <input
                    type="radio"
                    name="out-of-town-paper-size"
                    className="h-4 w-4 border-slate-300 text-emerald-600 focus:ring-emerald-500"
                    checked={paperSize === AUSF_PDF_PAGE_FORMAT.A4}
                    onChange={() => setPaperSize(OUT_OF_TOWN_PDF_PAGE_FORMAT.A4)}
                  />
                  A4
                </label>
                <label className="flex cursor-pointer items-center gap-1.5">
                  <input
                    ...
                    name="out-of-town-paper-size"
                    className="h-4 w-4 border-slate-300 text-emerald-600 focus:ring-emerald-500"
                    checked={paperSize === OUT_OF_TOWN_PDF_PAGE_FORMAT.LONG}
                    onChange={() => setPaperSize(OUT_OF_TOWN_PDF_PAGE_FORMAT.LONG)}
                  />
                  Long (8.5 in × 13 in)
                </label>
              </div>
      ...
      {isEditing ? (
        <div className="mx-auto mb-8 px-4 print:hidden" style={{ width: pageWidth, maxWidth: '100%' }}>
          <OutOfTownAffidavit embedded onClose={handleCloseEdit} />
        </div>
      ) : (
        <article ref={docArticleRef} className="mx-auto mb-8 print:mx-0 print:mb-0" style={{ width: pageWidth }}>
          <OutOfTownPrintDocument data={printData} variant={variant} />
        </article>
      )}
    </section>
  );
