...
export function AddApplicantModal({
  ...
  const submit = (e) => {
    ...
    childrenApi
      .create(payload)
      .then((res) => {
        toast.success(isColbBrap ? 'Late Registration (BRAP) record added.' : 'Late registration added.');
        onAdded?.();
        onClose();
      ...
      >
        <div className="border-b border-slate-200 px-6 py-4">
          <h2 id="add-applicant-modal-title" className="text-xl font-semibold text-slate-800">
            {isColbBrap ? 'Add Late Registration (BRAP)' : 'Add late registration'}
          </h2>
          <p className="mt-1 text-sm text-slate-500">
            {isColbBrap
              ? 'Fill out the profile for this Late Registration (BRAP) record.'
              : 'Fill out the late registration profile and apply any conditional requirements.'}
          </p>
        </div>
        ...
        <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col">
          ...
          <div className="flex flex-col-reverse gap-3 border-t border-slate-200 bg-white px-6 py-4 sm:flex-row sm:justify-end">
            ...
            >
              {loading ? 'Saving…' : isColbBrap ? 'Add Late Registration (BRAP)' : 'Add late registration'}
            </button>
          </div>
        </form>
      </div>
